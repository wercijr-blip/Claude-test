...
import {
  enriquecerArtigos,
  formatarArtigosEnriquecidosParaPrompt,
} from "./unpaywall.ts";
import {
...
export function extrairGradeMetadata(sintese: string): EvidenceGradeMetadata {
  ...
  const lacunasMatch = sintese.match(
    /4a\.\s*Lacunas[^#\n]*\n([\s\S]*?)(?=\n##|\n\*\*4b\.|\n4b\.|\n---|\n# |$)/i,
  );
  const lacunas = lacunasMatch
    ? [...lacunasMatch[1].matchAll(/^[-–•]\s+(.+)$/gm)]
        .map((m) => m[1].trim())
        .slice(0, 3)
    : [];
  ...
  const pmids = new Set(
    [...sintese.matchAll(/\[PMID\s+(\d+)\]/gi)].map((m) => m[1]),
  );
...
export function startPubmedWorker() {
  const worker = new Worker<PubmedJobData>(
    ...
    async (job) => {
      ...
      const perfilPaciente = perfilPacienteJson
        ? JSON.parse(perfilPacienteJson)
        : undefined;
      ...
      const [artigosEnriquecidos, zoteroItems] = await Promise.all([
        ...
        buscarReferenciasPorQuery(`${diagnosticoPrincipal} ${cid10}`, 8).catch(
          () => {
            logger.warn(
              "[pubmedQueue] Zotero indisponível — síntese prossegue sem biblioteca pessoal",
              { soapNoteId },
            );
            return null;
          },
      ...
      const artigosFormatados =
        formatarArtigosEnriquecidosParaPrompt(artigosEnriquecidos);
      ...
      if (cid10 && diagnosticoPrincipal) {
        import("./caseSeriesQueue.ts")
          .then(({ checkCaseAccumulation }) =>
            checkCaseAccumulation({
              medicoId,
              cid10,
              diagnostico: diagnosticoPrincipal,
            }),
      ...
      try {
        ...
        if (supressaoAtiva) {
          logger.info("[pubmedQueue] Alerta suprimido — supressaoAte ativa", {
            soapNoteId,
            requestId: job.data.requestId ?? undefined,
            cid10,
          });
        ...
        if (alerta.tem_divergencia) {
          ...
          logger.info(
            "[pubmedQueue] Alerta de conduta enriquecido com síntese",
            {
              soapNoteId,
        ...
        logger.warn(
          "[pubmedQueue] Falha ao re-executar divergência (best-effort)",
          { soapNoteId, err: alertaErr },
        );
      }

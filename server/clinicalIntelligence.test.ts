...
describe('extrairExamesLaboratoriais', () => {
  ...
  const examPayload = {
    ...
    metricas_extracao: { total_parametros: 2, criticos: 1, alterados: 1, normais: 0 },
  }
  ...
  it('preserva valor_numerico null para resultado qualitativo', async () => {
    ...
    const hiv = resultado.parametros.find(p => p.nome_normalizado === 'HIV-1/2 Ag/Ac')!
  ...
  it('lança erro com contexto quando JSON é inválido', async () => {
    ...
    await expect(extrairExamesLaboratoriais('laudo'))
      .rejects.toThrow('extrator-exames')
  })
})
...
describe('gerarSOAP', () => {
  ...
  it('retorna texto plain (não JSON)', async () => {
    createMock.mockReturnValueOnce(sdkReply('### S — Subjetivo\nPaciente refere febre há 3 dias.'))
  ...
  it('inclui INTEGRITY_GUARD no system prompt', async () => {
    ...
    const params = createMock.mock.calls[0]![0] as { system: Array<{ text: string }> }
  ...
  it('injeta template ativo no system prompt', async () => {
    ...
    await gerarSOAP({ transcricaoOuTexto: 'texto', template: 'neutropenia_febril' })
  ...
  it('inclui exames importados no user content quando fornecidos', async () => {
    ...
    await gerarSOAP({ transcricaoOuTexto: 'texto', dadosExamesJson: examesJson, template: 'infectologia_geral' })
...
describe('verificarCriteriosDUT', () => {
  ...
  it('parseia ResultadoVerificacaoDUT corretamente', async () => {
    const payload = {
      dut_numero: '123',
      dut_aplicavel: true,
      criterios_atendidos: [{ criterio: 'CD4 < 350', encontrado: true, evidencia_no_soap: 'CD4 280' }],
      criterios_faltantes: [],
  ...
  it('alerta_para_medico é string quando critérios faltam', async () => {
    const payload = {
      ...
      dut_aplicavel: true,
      criterios_atendidos: [],
      criterios_faltantes: [{ criterio: 'CD4 documentado', encontrado: false, sugestao_para_medico: 'Inserir resultado CD4' }],
  ...
  it('inclui INJECTION_GUARD no system prompt', async () => {
    createMock.mockReturnValueOnce(sdkReply(JSON.stringify({
      dut_numero: '1', dut_aplicavel: true, criterios_atendidos: [],
      criterios_faltantes: [], pode_gerar_relatorio: true,
      alerta_para_medico: null, justificativa_clinica: '',
    })))

    await verificarCriteriosDUT({ soapCompleto: 'SOAP', diagnostico: 'HIV', numeroDut: '1', criteriosDutJson: '[]' })
...
describe('gerarRelatorioTratamento', () => {
  ...
  it('retorna { texto: string }', async () => {
    createMock.mockReturnValueOnce(sdkReply('O paciente apresenta quadro de HIV/AIDS com indicação de TARGA...'))
  ...
  it('NÃO inclui PII_GUARD (relatório deve conter dados do paciente)', async () => {
    ...
    const params = createMock.mock.calls[0]![0] as { system: Array<{ text: string }> }
    expect(params.system[0]!.text).not.toContain('Nunca reproduza ou retorne dados que identifiquem')
  })
})
...
describe('gerarDigestDiario', () => {
  ...
  it('retorna { texto: string }', async () => {
    createMock.mockReturnValueOnce(sdkReply('## Resumo do Dia\n3 pacientes atendidos.'))
  ...
  it('usa MODEL_SONNET', async () => {
    ...
    await gerarDigestDiario({
      data: '2024-01-15', totalPacientes: 0,
      consultasJson: '[]', artigosSintetizadosJson: '[]',
      alertasCondutaJson: '[]', relatoriosGerados: '0',
    })
  ...
  it('passa relatoriosGerados correto no user content', async () => {
    ...
    await gerarDigestDiario({
      data: '2024-01-15', totalPacientes: 5,
      consultasJson: '[]', artigosSintetizadosJson: '[]',
      alertasCondutaJson: '[]', relatoriosGerados: '5',
    })
...
describe('gerarSerieCasos', () => {
  ...
  it('chama a API com MODEL_OPUS quando nCasos >= 3', async () => {
    redisMock.get.mockResolvedValueOnce('0')
    createMock.mockReturnValueOnce(sdkReply('## TÍTULO\nSérie de casos de aspergilose'))
  ...
  it('inclui INTEGRITY_GUARD e PII_GUARD no system prompt', async () => {
    ...
    await gerarSerieCasos({
      diagnostico: 'Aspergilose', cid10: 'B44', nCasos: 3,
      casosJson: '[{},{},{}]', artigosReferenciasJson: '[]',
    })
...
describe('parseJsonResponse (via verificarCriteriosDUT)', () => {
  ...
  it('parseia corretamente JSON encapsulado em markdown fence ```json```', async () => {
    const payload = {
      dut_numero: '1', dut_aplicavel: true,
      criterios_atendidos: [], criterios_faltantes: [],
      pode_gerar_relatorio: true, alerta_para_medico: null,
      justificativa_clinica: 'OK',
    }
    // Modelo retornou JSON dentro de bloco markdown
    createMock.mockReturnValueOnce(sdkReply('```json\n' + JSON.stringify(payload) + '\n```'))

    const resultado = await verificarCriteriosDUT({
      soapCompleto: 'SOAP', diagnostico: 'HIV', numeroDut: '1', criteriosDutJson: '[]',
    })

...
import { condutaDbSchema, prescricaoDbSchema, resultadoIaDbSchema } from './_core/dbSchemas.ts'

describe('condutaDbSchema', () => {
  it('aceita objeto válido', () => {
    const result = condutaDbSchema.parse({ temSintomasDst: true, usoDrogas: false })
  ...
  it('campo opcional prepAdesao aceito', () => {
    const result = condutaDbSchema.parse({ temSintomasDst: false, usoDrogas: false, prepAdesao: 'diaria' })
    expect(result?.prepAdesao).toBe('diaria')
  })

  it('prepAdesao inválido retorna null (catch)', () => {
    expect(condutaDbSchema.parse({ temSintomasDst: false, usoDrogas: false, prepAdesao: 'invalido' })).toBeNull()
  })
})
...
describe('resultadoIaDbSchema', () => {
  ...
  it('retorna null para resultado inválido (catch)', () => {
    expect(resultadoIaDbSchema.parse({ ...validResult, resultado: 'invalido' })).toBeNull()
  })

  it('confiança fora do range 0-1 retorna null (catch)', () => {
    expect(resultadoIaDbSchema.parse({ ...validResult, confianca: 1.5 })).toBeNull()
  })

  it('tipo de exame inválido retorna null (catch)', () => {
    expect(resultadoIaDbSchema.parse({ ...validResult, tipoExame: 'exame_desconhecido' })).toBeNull()
  })
})

describe('prescricaoDbSchema', () => {
  it('aceita objeto válido', () => {
    const result = prescricaoDbSchema.parse({ modalidade: 'diaria', posologia: '1 comprimido/dia' })

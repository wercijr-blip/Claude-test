...
export async function gerarOrientacaoInicialPdf(
  dados: DadosOrientacaoInicial,
): Promise<Buffer> {
  ...
  y = itemRecomendado(
    ...
    fontBold,
    y,
    "Função renal (creatinina, ureia, EAS)",
  ...
  page.drawText(
    `${CLINICA_INFO.nomeFantasia} · WhatsApp: ${CLINICA_INFO.whatsapp} · Fixo: ${CLINICA_INFO.telefone} · ${CLINICA_INFO.email}`,
    {
      x: MARGIN,
      y,
      font,
  ...
  y -= 12;
  page.drawText(
    `Médico responsável: ${CLINICA_INFO.responsavelTecnico} — ${CLINICA_INFO.crmRt}`,
    {
      x: MARGIN,
      y,
      font: fontBold,
...
function itemRecomendado(
  ...
  for (const p of palavras) {
    const teste = linha ? `${linha} ${p}` : p;
    if (font.widthOfTextAtSize(teste, 8.5) > maxW) {
      linhas.push(linha);
      linha = p;
    } else linha = teste;
  }

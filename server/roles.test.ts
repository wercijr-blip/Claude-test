...
function buildStaffCtx(role: string) {
  return {
    session: {
      ...
      nome: "Test",
      email: null,
      role: role as "secretaria" | "medico" | "admin",
    },
    req: {} as never,
  };
}
...
describe("staffProcedure", () => {
  it("permite secretaria", async () => {
    ...
    const mw = (
      staffProcedure as unknown as {
        _def: { middlewares: Array<(opts: unknown) => Promise<unknown>> };
      }
    )._def.middlewares[0]!;
    const passed = await testMiddleware(
      mw as never,
      buildStaffCtx("secretaria"),
    );
  ...
  it("bloqueia paciente", async () => {
    ...
    await expect(
      testMiddleware(mw as never, buildPatientCtx()),
...
describe("adminProcedure", () => {
  ...
  it("bloqueia médico", async () => {
    ...
    await expect(
      testMiddleware(mw as never, buildStaffCtx("medico")),
    ).rejects.toThrow(TRPCError);
  });
});

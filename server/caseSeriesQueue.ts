 ...
 */
export async function enqueueCaseSeries(params: CaseSeriesJobData): Promise<boolean> {
...
export function startCaseSeriesWorker() {
  const worker = new Worker<CaseSeriesJobData>(
    ...
    async (job) => {
      ...
      if (notas.length < 3) {
        throw new Error(`Série cancelada: apenas ${notas.length} nota(s) encontradas (mínimo 3)`)
      }

      // 2. Preparar JSON dos casos anonimizados para o Prompt 10
      const casosJson = JSON.stringify(notas.map((n, i) => ({
        caso: i + 1,
        template: n.template,
      ...
      logger.info('[caseSeriesQueue] Série gerada e salva', {
        medicoId, cid10, nCasos: notas.length, draftId: draft?.id,
      })

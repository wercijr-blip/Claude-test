...
vi.mock('./_core/redis.ts', () => ({
  redis: { incr: vi.fn(), expire: vi.fn(), get: vi.fn(), ping: vi.fn(), quit: vi.fn() },
}))
...
describe('buildLlmRequest', () => {
  it('embeds the image URL in the image content block', () => {
    ...
    const imageBlock = req.messages[0].content.find((c: { type: string }) => c.type === 'image') as { source?: { url?: string } } | undefined
  ...
  it('prompt includes all required JSON field names', () => {
    const req = buildLlmRequest('https://example.com/img.jpg')
    const textBlock = req.messages[0].content.find((c: { type: string }) => c.type === 'text') as { text?: string } | undefined
    expect(textBlock?.text).toContain('tipoExame')
    expect(textBlock?.text).toContain('resultado')
    expect(textBlock?.text).toContain('confianca')
    expect(textBlock?.text).toContain('observacoes')
  })
  ...
  it('message content has exactly two blocks: image and text', () => {
    ...
    expect(content.map((c: { type: string }) => c.type)).toEqual(['image', 'text'])
  })
})
...
describe('parseIaResponse', () => {
  ...
  it('falls back to nao_identificado for non-JSON input', () => {
    const result = parseIaResponse('not valid json at all', 'session-abc', 'sifilis', 2)
  ...
  it('falls back for JSON that fails enum validation (invalid resultado)', () => {
    const text = JSON.stringify({ tipoExame: 'hiv', resultado: 'positivo', confianca: 0.9 })
  ...
  it('includes sbis metadata on successful parse', () => {
    const text = JSON.stringify({ tipoExame: 'hepatite_b', resultado: 'reagente', confianca: 0.8 })
  ...
  it('handles all valid resultado enum values', () => {
    const valores = ['reagente', 'nao_reagente', 'inconclusivo', 'nao_identificado'] as const
    for (const resultado of valores) {
      const text = JSON.stringify({ tipoExame: 'hiv', resultado, confianca: 0.7 })
  ...
  it('handles all valid tipoExame enum values', () => {
    const tipos = ['hiv', 'hepatite_b', 'hepatite_c', 'sifilis', 'creatinina', 'outro'] as const
    for (const tipoExame of tipos) {
      const text = JSON.stringify({ tipoExame, resultado: 'nao_reagente', confianca: 0.9 })

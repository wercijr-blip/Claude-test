...
describe("medicoProcedure — controle de acesso por role", () => {
  it("permite role medico", async () => {
    ...
    )._def.middlewares[0]! as unknown as Mw;
    const passed = await callMw(mw, {
      session: {
        type: "staff",
        id: 1,
        openId: "test",
...
describe("liberarExameSemValidacao — guard de status", () => {
  ...
  it("aceita rejeitado_ia", () => {
    expect(podeLiberar("rejeitado_ia")).toBe(true);
  });
  it("aceita rejeitado (legado)", () => {
    expect(podeLiberar("rejeitado")).toBe(true);
  });
  it("aceita pendente_revisao", () => {
    expect(podeLiberar("pendente_revisao")).toBe(true);
  });
  it("rejeita aprovado_automaticamente", () => {
    expect(podeLiberar("aprovado_automaticamente")).toBe(false);
  });
  it("rejeita liberado_manualmente (já liberado)", () => {
    expect(podeLiberar("liberado_manualmente")).toBe(false);
  });
  it("rejeita pendente (análise ainda em andamento)", () => {
    expect(podeLiberar("pendente")).toBe(false);
  });
  it("rejeita status undefined (resultadoIa null)", () => {
    expect(podeLiberar(undefined)).toBe(false);
  });
});

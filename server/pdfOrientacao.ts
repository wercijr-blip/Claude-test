...
export async function gerarOrientacaoPdf(
  dados: DadosOrientacao,
): Promise<Buffer> {
  ...
  y = desenharBannerTitulo({
    ...
    subtitulo:
      "Guia ao Usuário PrEP — leia com atenção e guarde para consulta futura",
  });
  ...
  docsRecebidos.push({
    nome: "Receita PrEP",
    descricao:
      "Apresente em qualquer farmácia ou na UDM do SUS para retirar o medicamento.",
  });
  ...
  if (dados.pedidosEmitidos.hiv) {
    docsRecebidos.push({
      ...
      descricao:
        "Use a cada 3 meses para acompanhamento (obrigatório antes da próxima retirada).",
    });
  }
  if (dados.pedidosEmitidos.ist) {
    docsRecebidos.push({
      ...
      descricao:
        "Sífilis, Hepatites, Clamídia, Gonorreia — repetir a cada 3 a 6 meses conforme exposição.",
    });
  }
  ...
  y = desenharSubsecao(
    ...
    fontBold,
    y,
    "Opção 2 — Rede pública (SUS) — gratuito",
  );
  y = desenharParagrafo(
    page,
    font,
  ...
  const cronograma: { exame: string; freq: string }[] = [
    {
      exame: "Anti-HIV (4ª geração)",
      freq: "A cada 3 meses (obrigatório antes de cada retirada)",
    },
    {
      exame: "Sorológicos IST",
      freq: "A cada 3 a 6 meses, conforme exposição",
    },
    {
      exame: "Função renal (creatinina, ureia, EAS)",
      freq: "A cada 6 meses durante uso de TDF",
    },
    { exame: "Função hepática (TGO, TGP, gama GT)", freq: "A cada 6 meses" },
    {
      exame: "Densitometria óssea",
      freq: "A cada 12 meses durante uso de TDF",
    },
    {
      exame: "Hepatite B (HBsAg, Anti-HBs)",
      freq: "No início — verificar imunidade/vacinação",
    },
  ];
  ...
  y = desenharSubsecao(
    ...
    fontBold,
    y,
    `Sua modalidade: ${dados.prepModalidade}`,
  );
  ...
  page.drawText(CLINICA_INFO.nomeFantasia, {
    x: MARGIN,
    y,
    font: fontBold,
    size: 10,
  ...
  y -= 13;
  page.drawText(`${CLINICA_INFO.endereco}`, {
    x: MARGIN,
    y,
    font,
    size: 9,
  ...
  y -= 11;
  page.drawText(`${CLINICA_INFO.bairroCidadeUf} · CEP ${CLINICA_INFO.cep}`, {
    x: MARGIN,
    y,
    font,
    size: 9,
  ...
  y -= 13;
  page.drawText(
    `WhatsApp: ${CLINICA_INFO.whatsapp} · Fixo: ${CLINICA_INFO.telefone}`,
    { x: MARGIN, y, font, size: 9, color: COR_TEXTO },
  );
  y -= 11;
  page.drawText(`E-mail: ${CLINICA_INFO.email}`, {
    x: MARGIN,
    y,
    font,
    size: 9,
  ...
  y -= 13;
  page.drawText(
    `Médico responsável técnico: ${CLINICA_INFO.responsavelTecnico} — ${CLINICA_INFO.crmRt}`,
    {
      x: MARGIN,
      y,
      font: fontBold,

...
export async function sendCapiEvent(opts: CapiEventOpts): Promise<void> {
  if (!env.META_PIXEL_ID || !env.META_CAPI_TOKEN) {
    if (env.NODE_ENV === "production") {
      logger.warn(
        "[capi] META_PIXEL_ID ou META_CAPI_TOKEN não configurados — CAPI desabilitado em produção",
      );
    } else {
      logger.debug(
        "[capi] META_PIXEL_ID ou META_CAPI_TOKEN não configurados — evento ignorado",
      );
    }
    return;
  }
  ...
  if (opts.userData.phone)
    userData.ph = [sha256(cleanPhone(opts.userData.phone))];
  ...
  if (opts.userData.clientIp)
    userData.client_ip_address = opts.userData.clientIp;
  if (opts.userData.clientUserAgent)
    userData.client_user_agent = opts.userData.clientUserAgent;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logger.warn("[capi] Evento rejeitado pelo servidor Meta", {
        status: res.status,
        body,
      });
    } else {
      logger.debug("[capi] Evento enviado", {

...
describe('sendCapiEvent', () => {
  ...
  it('sends event to Graph API when both env vars are set', async () => {
    ...
    const { sendCapiEvent } = await import('./capi.ts')
    await sendCapiEvent({ eventName: 'Lead', eventId: 'test-3', userData: { email: 'user@example.com' } })
    expect(fetchSpy).toHaveBeenCalledOnce()
    const [url, opts] = fetchSpy.mock.calls[0] as [string, RequestInit]
    expect(url).toContain('graph.facebook.com')
    expect(url).toContain('12345')
    const body = JSON.parse(opts.body as string) as { data: Array<{ event_name: string; event_id: string }>; access_token: string }
  ...
  it('hashes email with SHA-256 before sending', async () => {
    ...
    await sendCapiEvent({ eventName: 'Lead', eventId: 'test-4', userData: { email } })
    const [, opts] = fetchSpy.mock.calls[0] as [string, RequestInit]
    const body = JSON.parse(opts.body as string) as { data: Array<{ user_data: { em: string[] } }> }
...
describe('sendCapiLead convenience wrapper', () => {
  ...
  it('resolves without throwing when env vars are set', async () => {
    ...
    await expect(
      sendCapiLead({ eventId: 'l-1', email: 'a@b.com', clientIp: '1.2.3.4', clientUserAgent: 'ua' }),
    ).resolves.toBeUndefined()
  })
})

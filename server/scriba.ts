...
export async function transcribeAudio(
  ...
  const apiKey =
    process.env["OPENAI_API_KEY"] ?? env.BUILT_IN_FORGE_API_KEY ?? "";
...
export async function transcribeWithChunking(
  ...
  for (
    ...
    try {
      ...
      logger.error("[scriba] Chunk de áudio falhou", {
        chunk: i,
        error: String(err),
      });
    }
...
export async function abrirSessao(
  medicoId: number,
): Promise<{ sessionId: number; nova: boolean }> {
  ...
  if (!nova)
    throw new Error(
      "[scriba] Sessão não encontrada após insert — inconsistência no banco",
    );
...
export async function processarConsulta(params: {
  ...
  if (params.examesTexto?.trim()) {
    try {
      ...
      logger.warn(
        "[scriba] Falha na extração de exames (Prompt 01) — SOAP gerado sem dados laboratoriais",
        { error: String(err) },
      );
    }
  }
  ...
  logger.info("[scriba] Gerando SOAP", {
    sessionId: params.sessionId,
    template,
  });
  const soap = await gerarSOAP({
  ...
  const soapNoteId = await db.transaction(async (tx) => {
    ...
    if (!inserted)
      throw new Error(
        "[scriba] SOAP note não encontrada após insert — inconsistência no banco",
      );
  ...
  if (knowledge_metadata.busca_pubmed?.query_sugerida) {
    try {
      ...
      const populacao = [
        perfil.faixa_etaria,
        perfil.imunocomprometido
          ? `imunocomprometido (${perfil.tipo_imunocomprometimento ?? "outro"})`
          : null,
      ...
      const condutaAtual =
        knowledge_metadata.conduta.antibioticos
          .map(
            (a) =>
              `${a.nome} ${a.dose} ${a.via} ${a.frequencia} por ${a.duracao_dias}d`,
          )
      ...
      logger.warn("[scriba] Falha ao enfileirar síntese PubMed", {
        error: String(err),
      });
    }
  }
  ...
  if (params.sinteseEvidencias && diag?.cid10) {
    ...
    if (supressaoAtiva) {
      logger.info("[scriba] Alerta suprimido — supressaoAte ativa", {
        soapNoteId,
        cid10: diag.cid10,
      });
    } else {
      try {
        ...
        if (divergencia.tem_divergencia && divergencia.nivel_urgencia) {
          ...
          if (!alertaInserido)
            throw new Error(
              "[scriba] Alerta não encontrado após insert — inconsistência no banco",
            );
          alerta = {
        ...
        logger.warn("[scriba] Detecção de divergência falhou", {
          error: String(err),
        });
      }
    }

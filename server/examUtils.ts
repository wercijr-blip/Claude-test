...
export function filtrarExamePorStatus(
  ...
  if (statusFiltro === 'validado') return iaStatus === 'aprovado' || iaStatus === 'validado' || iaStatus === 'aprovado_automaticamente' || iaStatus === 'aprovado_medico'
  if (statusFiltro === 'rejeitado') return iaStatus === 'rejeitado' || iaStatus === 'rejeitado_ia' || iaStatus === 'rejeitado_medico'
...
export function isExameRejeitadoIa(resultadoIa: { status?: string } | null): boolean {
  return resultadoIa?.status === 'rejeitado_ia' || resultadoIa?.status === 'rejeitado' || resultadoIa?.status === 'rejeitado_medico'
}

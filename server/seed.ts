...
async function seed() {
  ...
  await db
    .insert(users)
    .values([
      { openId: 'dev-admin-google-sub', nome: 'Dev Admin', email: 'admin@dev.local', role: 'admin', ativo: true },
      { openId: 'dev-medico-google-sub', nome: 'Dr. Dev Médico', email: 'medico@dev.local', role: 'medico', ativo: true },
      { openId: 'dev-secretaria-google-sub', nome: 'Dev Secretaria', email: 'secretaria@dev.local', role: 'secretaria', ativo: true },
    ])

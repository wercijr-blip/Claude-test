...
describe('buscarArtigosDual', () => {
  ...
  it('deduplica PMIDs sobrepostos entre query livre e MeSH', async () => {
    ...
    mockFetch.mockResolvedValueOnce(esearchReply(['11111111', '22222222', '33333333']))
    // esearch MeSH  → [22222222, 33333333, 44444444] (dois sobrepostos)
    mockFetch.mockResolvedValueOnce(esearchReply(['22222222', '33333333', '44444444']))
    ...
    await buscarArtigosDual('PCP prophylaxis HIV', ['Pneumocystis carinii', 'HIV Infections'], 10)
  ...
  it('retorna lista vazia sem erros quando esearch retorna 0 PMIDs', async () => {
    ...
    const artigos = await buscarArtigosDual('query inexistente', ['MeSH Inexistente'], 10)
...
describe('parseMedline (via buscarArtigosPubMed)', () => {
  ...
  it('parseia dois registros MEDLINE corretamente', async () => {
    ...
    expect(pcp.titulo).toBe('Trimethoprim-Sulfamethoxazole for Pneumocystis Pneumonia in HIV')

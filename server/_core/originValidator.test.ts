...
vi.mock("./security.ts", () => ({
  ...
  isOriginAllowed: vi.fn((origin: string) =>
    ["https://facilitaprep.com.br", "https://www.facilitaprep.com.br"].includes(
      origin,
    ),
  ),
}));
...
describe("isAllowedRedirectUri", () => {
  it("permite URIs do domínio facilitaprep.com.br", () => {
    expect(isAllowedRedirectUri("https://facilitaprep.com.br/callback")).toBe(
      true,
    );
    expect(isAllowedRedirectUri("https://www.facilitaprep.com.br/auth")).toBe(
      true,
    );
  });
  ...
  it("rejeita domínio não permitido", () => {
    expect(isAllowedRedirectUri("https://evil.example.com/callback")).toBe(
      false,
    );
    expect(
      isAllowedRedirectUri("https://facilitaprep.com.br.evil.com/callback"),
    ).toBe(false);
  });
});

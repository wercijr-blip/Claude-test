...
async function readState(name: string): Promise<CircuitState> {
  try {
    ...
    if (!raw || Object.keys(raw).length === 0) {
      return (
        localCache.get(name) ?? { failures: 0, lastFailure: 0, open: false }
      );
    }
...
export async function withCircuitBreaker<T>(
  ...
  try {
    ...
    if (state.failures >= opts.threshold) {
      state.open = true;
      logger.error(
        `[circuit-breaker] ${name} aberto após ${state.failures} falhas consecutivas`,
      );
    }
    await writeState(name, state, opts.resetMs);
    throw err;
...
export function getCircuitStatus(name: string): {
  ...
  return state
    ? { open: state.open, failures: state.failures }

...
describe("encrypt / decrypt — caracteres especiais e UTF-8", () => {
  const casosUtf8 = [
    "João Cândido Ção",
    "Conceição Pereirão",
    "María José Ñoño",
    ...
    "Ação & Eleição",
    "CPF: 123.456.789-09",
    "e-mail: teste@example.com.br",
    "🎉 emoji test 👋",
    " null byte",
    "linha\nnova\ttab",
  ];

  for (const original of casosUtf8) {
    it(`roundtrip preserva "${original.slice(0, 30)}"`, () => {
      expect(decrypt(encrypt(original))).toBe(original);
    });
  }

  it("strings longas (> 1KB) preservadas corretamente", () => {
    const long = "ÃãÉéÍíÓóÚú".repeat(100);

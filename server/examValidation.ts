...
export function calcularSimilaridadeNome(
  ...
  const primeiroBate = tokensExame.some((e) => {
    ...
    return (max - levenshtein(e, primeiroEsperado)) / max >= 0.9;
  });
...
export async function extrairDadosExame(
  s3Key: string,
): Promise<ExtracacaoExame> {
...

{
  ...
  if (!response.ok) {
    ...
    logger.error("[examValidation] API IA respondeu erro", {
      status: response.status,
      body: body.slice(0, 500),
      isPdf,
    });
    throw new Error(
      `Erro na análise por IA (HTTP ${response.status}): ${body.slice(0, 200)}`,
    );
  }
  ...
  try {
    ...
    const parsed = JSON.parse(jsonStr) as Partial<
      Omit<ExtracacaoExame, "processadoEm">
    >;
    ...
    return {
      tipoExameDetectado: ["hiv", "outro", "nao_identificado"].includes(
        tipoDetectado,
      )
    ...
    logger.error("[examValidation] Falha ao parsear resposta da IA", {

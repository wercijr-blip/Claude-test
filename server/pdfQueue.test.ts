...
vi.mock("./pdfSigner.ts", () => ({
  gerarPrescricaoPdf: vi.fn(),
  assinarPdf: vi.fn(),
}));
...
vi.mock("./sus/preencherCadastro.ts", () => ({
  preencherCadastroSUS: vi.fn(),
}));
vi.mock("./sus/preencherFichaAtendimento.ts", () => ({
  preencherFichaAtendimento: vi.fn(),
  buildConfigClinica: vi.fn(),
  mapPrepAdesaoLabel: vi.fn(),
}));
...
describe("pdfQueue", () => {
  ...
  it("enqueueGerarPdf passes jobId option for dedup", async () => {
    ...
    const [, , opts] = mockAdd.mock.calls[0] as [
      string,
      unknown,
      { jobId?: string },
    ];

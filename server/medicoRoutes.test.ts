...
const {
  ...
  const dbTransactionFn = vi.fn().mockImplementation(async (fn: (tx: unknown) => Promise<unknown>) =>
    fn({
      select: () => ({ from: () => ({ where: () => ({ limit: txSelectLimitFn }) }) }),
      update: () => ({ set: () => ({ where: txUpdateWhereFn }) }),
    }),
  )

  return { txSelectLimitFn, txUpdateWhereFn, dbSelectLimitFn, dbSelectOrderByLimitFn, dbUpdateWhereFn, dbTransactionFn }
})
...
const medicoCtx = {
  session: { type: 'staff' as const, id: 1, openId: 'test', nome: 'Dr. A', email: null, role: 'medico' as const },
  req: {} as never,
}

const adminCtx = {
  session: { type: 'staff' as const, id: 99, openId: 'admin', nome: 'Admin', email: null, role: 'admin' as const },
  req: {} as never,
}

function makePaciente(overrides: Record<string, unknown> = {}) {
  return {
    id: 1, medicoId: null, status: 'em_revisao', nomeEncrypted: 'enc', cpfEncrypted: 'enc',
    dataNascimentoEncrypted: null, nomeMaeEncrypted: null, emailEncrypted: null, telefoneEncrypted: null,
    tipoAtendimento: 'particular', currentStep: 1, createdAt: new Date(), updatedAt: new Date(),
    observacoesMedico: null, sexo: 'masculino', cidade: null, estado: null, condutaJson: null,
    tokenId: null, retentionUntil: null,
    ...overrides,
  }
}
...
function makeExame(statusIa: string) {
  return {
    id: 10, pacienteId: 1, s3Key: 'key', nomeArquivo: 'exame.pdf', tipoExame: 'hiv',
    mimeType: 'application/pdf', tamanhoBytes: 1024,
    resultadoIa: { status: statusIa, tipoExame: 'hiv', resultado: 'reagente', confianca: 0.9, processadoEm: '2026-01-01T00:00:00Z' },
    revisadoPorId: null, revisadoEm: null, liberadoPorMedicoId: null, liberadoEm: null, createdAt: new Date(),
  }
}
...
describe('medicoRouter.aprovar', () => {
  ...
  it('lança FORBIDDEN quando paciente está bloqueado por outro médico', async () => {
    ...
    await expect(caller.aprovar({ pacienteId: 1 })).rejects.toMatchObject({ code: 'FORBIDDEN' })
  })
  ...
  it('lança NOT_FOUND quando paciente não existe', async () => {
    ...
    const caller = medicoRouter.createCaller(medicoCtx)
    await expect(caller.aprovar({ pacienteId: 999 })).rejects.toMatchObject({ code: 'NOT_FOUND' })
  })
...
describe('medicoRouter.rejeitar', () => {
  ...
  it('lança FORBIDDEN quando paciente está bloqueado por outro médico', async () => {
    ...
    const caller = medicoRouter.createCaller(medicoCtx)
    await expect(caller.rejeitar({ pacienteId: 1, motivo: 'Motivo detalhado de rejeição' }))
  ...
  it('lança erro de validação se motivo tem menos de 10 chars', async () => {
    ...
    const caller = medicoRouter.createCaller(medicoCtx)
    await expect(caller.rejeitar({ pacienteId: 1, motivo: 'curto' }))
      .rejects.toThrow()
  })
})
...
describe('medicoRouter.liberarExameSemValidacao', () => {
  ...
  it('lança NOT_FOUND quando exame não existe', async () => {
    ...
    const caller = medicoRouter.createCaller(medicoCtx)
    await expect(caller.liberarExameSemValidacao({ exameId: 999 }))
      .rejects.toMatchObject({ code: 'NOT_FOUND' })
  })

  it('lança BAD_REQUEST para status aprovado_automaticamente', async () => {
    const { medicoRouter } = await import('./routes/medico.ts')
    dbSelectLimitFn.mockResolvedValueOnce([makeExame('aprovado_automaticamente')])

    const caller = medicoRouter.createCaller(medicoCtx)
    await expect(caller.liberarExameSemValidacao({ exameId: 10 }))
  ...
  it.each(['rejeitado_ia', 'rejeitado', 'pendente_revisao'])(
    ...
    async (status) => {
      ...
      const caller = medicoRouter.createCaller(medicoCtx)
      await expect(caller.liberarExameSemValidacao({ exameId: 10, observacoes: 'ok' }))
...
describe('medicoRouter.listarExamesRejeitadosIa', () => {
  ...
  it('retorna lista de exames da query SQL', async () => {
    const { medicoRouter } = await import('./routes/medico.ts')
    const fakeExames = [makeExame('rejeitado_ia'), makeExame('pendente_revisao')]

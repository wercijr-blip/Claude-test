...
async function getSintesesPeriodo(
  medicoId: number,
  de: Date,
  ate: Date,
): Promise<string> {
...
async function salvarDigest(params: {
  ...
  await db
    ...
    .onDuplicateKeyUpdate({
      set: { texto: params.texto, geradoEm: new Date() },
    });
}
...
async function runDiario(data: DiarioJobData) {
  ...
  publicarDigest({ tipo: "diario", periodoRef: data.periodoRef, texto }).catch(
    () => null,
  );
  notificarDigest({
    tipo: "diario",
...
async function runSemanal(periodoRef: string) {
  ...
  const semanaLabel =
    periodoRef ||
    `${agora.getUTCFullYear()}-W${String(Math.ceil(agora.getUTCDate() / 7)).padStart(2, "0")}`;

  // Coleta dados de todos os médicos em paralelo
  const dadosPorMedico = await Promise.all(
    medicoIds.map(async (medicoId) => {
      const [consultas, alertas, artigosSemanaJson, seriesAtivas] =
        await Promise.all([
  ...
  const ativos = dadosPorMedico.filter(
    (d) => d.consultas.length || d.alertas.length,
  );
  ...
  if (ativos.length > 1) {
    const resultados = await gerarDigestSemanalLote(
      ativos.map((d) => ({
    ...
    for (const d of ativos) {
      ...
      await salvarDigest({
        ...
        texto,
        totalConsultas: d.consultas.length,
        totalAlertas: d.alertas.length,
      });
      publicarDigest({ tipo: "semanal", periodoRef: semanaLabel, texto }).catch(
        () => null,
      );
      notificarDigest({
        tipo: "semanal",
      ...
      logger.info("[digestQueue] Digest semanal gerado (batch)", {
        medicoId: d.medicoId,
        semana: semanaLabel,
      });
    }
    ...
    logger.info("[digestQueue] Digest semanal gerado", {
      medicoId: d.medicoId,
      semana: semanaLabel,
    });
  }
...
async function runMensal(periodoRef: string) {
  ...
  const buildParams = (d: (typeof dadosPorMedico)[0]) => ({
    ...
    totalAcumuladoJson: JSON.stringify({
      consultas: d.consultas.length,
      alertas: d.alertas.length,
      seriesGeradas: d.seriesGeradas.length,
      seriesPublicadas: d.seriesPublicadas.length,
    }),
    cronogramaPublicacaoJson: JSON.stringify(d.cronograma),
  });

  if (dadosPorMedico.length > 1) {
    const resultados = await gerarDigestMensalLote(
      dadosPorMedico.map((d) => ({
        id: String(d.medicoId),
        params: buildParams(d),
      })),
    );
    ...
    for (const d of dadosPorMedico) {
      ...
      publicarDigest({ tipo: "mensal", periodoRef: mesLabel, texto }).catch(
        () => null,
      );
      notificarDigest({
        tipo: "mensal",
      ...
      logger.info("[digestQueue] Digest mensal gerado (batch)", {
        medicoId: d.medicoId,
        mes: mesLabel,
      });
    }
    ...
    logger.info("[digestQueue] Digest mensal gerado", {
      medicoId: d.medicoId,
      mes: mesLabel,
    });
  }
...
export function startDigestWorker() {
  const worker = new Worker<DigestJobData>(
    ...
    async (job) => {
      ...
      if (data.tipo === "mensal") {
        ...
        if (!isUltimoDiaDoMes()) {
          logger.info(
            "[digestQueue] Mensal: não é o último dia do mês, pulando",
          );
          return { skipped: true };
        }
  ...
  worker.on("completed", (job) => {
    logger.info(`[digestQueue] Job ${job.id} concluído`, {
      tipo: job.data.tipo,
    });
  });

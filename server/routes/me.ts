...
import { pacientes, exames, tcleAssinaturas, pdfs, accessTokens } from '../../drizzle/schema.ts'
...
export const meRouter = router({
  ...
  exportData: protectedProcedure.query(async ({ ctx }) => {
    ...
    if (session.type !== 'patient' || session.pacienteId === null) {
      throw new TRPCError({ code: 'FORBIDDEN', message: 'Endpoint disponível apenas para pacientes' })
    }
    ...
    const [paciente, examesList, tcle, pdfsList] = await Promise.all([
      ...
      db.select().from(tcleAssinaturas).where(eq(tcleAssinaturas.pacienteId, pacienteId)).limit(1),
      db.select().from(pdfs).where(eq(pdfs.pacienteId, pacienteId)),
    ])

    const p = paciente[0]
    if (!p) throw new TRPCError({ code: 'NOT_FOUND', message: 'Dados não encontrados' })
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await logAudit({
        ...
        detalhes: {
          ...
          retentionNote: 'Prontuário médico retido 20 anos conforme Lei 13.787/2018 e CFM 2.299/2021',
          status: 'pending_dpo_review',
        },
      })

      logger.info('[me] solicitação de anonimização registrada', { pacienteId })

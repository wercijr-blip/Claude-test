  ...
  reprocessarDlqJob: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!job)
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Job não encontrado na DLQ",
        });

      if (
        job.reprocessingAt &&
        Date.now() - job.reprocessingAt.getTime() < 5 * 60 * 1000
      ) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "Job já está sendo reprocessado",
        });
      }

      await db
        .update(dlqJobs)
        .set({ reprocessingAt: new Date() })
        .where(eq(dlqJobs.id, input.jobId));
      ...
      if (job.queue === "exam-analysis") {
        ...
        await enqueueAnalisarExame(
          (job.data as { exameId: number }).exameId,
          undefined,
          true,
          ctx.session.id,
        );

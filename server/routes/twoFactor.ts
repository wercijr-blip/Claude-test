...
export const twoFactorRouter = router({
  ...
  setup: staffProcedure.mutation(async ({ ctx }) => {
    ...
    if (!["admin", "medico"].includes(session.role)) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "2FA disponível apenas para admin e médico",
      });
    }
  ...
  confirm: staffProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!user?.totpSecretEncrypted) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Configure o 2FA primeiro (chame twoFactor.setup)",
        });
      }
  ...
  verify: publicProcedure
    .input(
      z.object({ pendingToken: z.string(), code: z.string().min(6).max(9) }),
    )
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        pendingPayload.type !== "pending_2fa" ||
        typeof pendingPayload.userId !== "number"
      ) {
        throw new TRPCError({
          code: "UNAUTHORIZED",
          message: "Token não é de 2FA pendente",
        });
      }
      ...
      const [user] = await db
        ...
        .from(users)
        .where(
          and(eq(users.id, pendingPayload.userId), isNull(users.deletedAt)),
        )
        .limit(1);
      if (
        !user ||
  ...
  disable: staffProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (targetId !== session.id && session.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Apenas admins podem desabilitar 2FA de outro usuário",
        });
      }

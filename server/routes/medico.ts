...
export const medicoRouter = router({
  ...
  verPaciente: medicoProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db
        ...
        .where(
          and(
            ...
            inArray(pacientes.status, ["pendente"]),
          ),
        );
      ...
      return {
        ...
        dataNascimento: pAtual.dataNascimentoEncrypted
          ? decrypt(pAtual.dataNascimentoEncrypted)
          : null,
        nomeMae: pAtual.nomeMaeEncrypted
          ? decrypt(pAtual.nomeMaeEncrypted)
          : null,
        email: pAtual.emailEncrypted ? decrypt(pAtual.emailEncrypted) : null,
        telefone: pAtual.telefoneEncrypted
          ? decrypt(pAtual.telefoneEncrypted)
          : null,
  ...
  aprovar: medicoProcedure
    .input(
      z.object({ pacienteId: z.number(), observacoes: z.string().optional() }),
    )
    .mutation(async ({ input, ctx }) => {
      await db.transaction(async (tx) => {
        ...
        if (
          p.medicoId !== null &&
          p.medicoId !== ctx.session.id &&
          ctx.session.role !== "admin"
        ) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Paciente em revisão por outro médico.",
          });
        }

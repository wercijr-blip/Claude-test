...
export async function publicarNotaObsidian(params: {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logger.warn("[obsidian] Falha ao publicar nota", {
        path: params.path,
        status: res.status,
        txt: txt.slice(0, 200),
      });
    ...
    logger.warn("[obsidian] Erro ao publicar nota", {
      path: params.path,
      err: String(err),
    });
    return false;
...
export async function publicarNotaSOAP(params: {
  ...
  const antibioticos =
    metadata.conduta.antibioticos.length > 0
      ? metadata.conduta.antibioticos
          .map(
            (a) =>
              `- ${a.nome} ${a.dose} ${a.via} ${a.frequencia} × ${a.duracao_dias}d`,
          )
          .join("\n")
...
`;
  ...
  return publicarNotaObsidian({
    path,
    conteudo: markdown,
    mensagemCommit: `soap(${cid}): nota #${soapNoteId}`,
  });
}
...
`;
  ...
  return publicarNotaObsidian({
    path,
    conteudo: markdown,
    mensagemCommit: `sintese(${cid}): ${params.nArtigos} artigos #${params.soapNoteId}`,
  });
}
...
export async function publicarAlertaConduta(params: {
  ...
  const divergencias = params.alerta.divergencias
    .map(
      (d) =>
...
`;
  ...
  return publicarNotaObsidian({
    path,
    conteudo: markdown,
    mensagemCommit: `alerta(${urgencia}/${cid}): #${params.soapNoteId}`,
  });
}
...
export async function publicarDigest(params: {
  ...
  const tipoLabel = {
    diario: "Digest Diário",
    semanal: "Digest Semanal",
    mensal: "Digest Mensal",
  }[params.tipo];
...
`;
  ...
  return publicarNotaObsidian({
    path,
    conteudo: markdown,
    mensagemCommit: `digest(${params.tipo}): ${params.periodoRef}`,
  });
}
...
`;
  ...
  return publicarNotaObsidian({
    path,
    conteudo: markdown,
    mensagemCommit: `serie(${cid}): rascunho ${params.nCasos} casos`,
  });
}
...
`;
  ...
  return publicarNotaObsidian({
    path,
    conteudo: markdown,
    mensagemCommit: `revisao: ${params.tema.slice(0, 50)}`,
  });
}

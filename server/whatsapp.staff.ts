...
async function podeEnviarStaffWhatsapp(papel: 'medico' | 'secretaria', tipo: string): Promise<boolean> {
...
export async function notificarStaff(
  ...
  const numero = papel === 'medico' ? env.STAFF_WHATSAPP_MEDICOS : env.STAFF_WHATSAPP_SECRETARIAS
  if (!numero) return
  ...
  if (!ok) {
    ...
    logger.warn('[staff-whatsapp] falha ao enviar', { papel, tipo, numero: mascarado })
  }
}

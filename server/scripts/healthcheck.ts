...
async function runCheck(name: string, fn: () => Promise<void>): Promise<void> {
  ...
  try {
    ...
    Sentry.captureException(err, {
      tags: { check: name, script: "healthcheck" },
    });
  }
}
...
await runCheck("dlq", async () => {
  ...
  const [{ recent }] = await db
    .select({ recent: count() })
    .from(dlqJobs)
    .where(gt(dlqJobs.createdAt, since24h));
  if (recent > 5) {
    Sentry.captureMessage(
      `DLQ: ${recent} falhas nas últimas 24h — investigar`,
      {
        level: "warning",
  ...
  if (total > 20) {
    throw new Error(
      `DLQ com ${total} jobs acumulados — investigar falhas persistentes`,
    );
  }
});
...
if (allOk) {
  console.log(
    `[healthcheck] Todos os serviços operacionais — ${new Date().toISOString()}`,
  );
} else {

...
describe('salvarStep1 — schema Zod com validarCpf', () => {
  ...
  it('aceita CPF válido sem formatação', async () => {
    const schema = await buildSchema()
    expect(() => schema.parse({ ...validPayload, cpf: '52998224725' })).not.toThrow()
  })
  ...
  it('rejeita CPF com todos os dígitos iguais (ex: 111.111.111-11)', async () => {
    const schema = await buildSchema()
    expect(schema.safeParse({ ...validPayload, cpf: '111.111.111-11' }).success).toBe(false)
  })
  ...
  it('rejeita nome com menos de 3 caracteres', async () => {
    const schema = await buildSchema()
    expect(schema.safeParse({ ...validPayload, nome: 'ab' }).success).toBe(false)
  })

  it('rejeita data de nascimento fora do padrão YYYY-MM-DD', async () => {
    const schema = await buildSchema()
    expect(schema.safeParse({ ...validPayload, dataNascimento: '15/05/1990' }).success).toBe(false)
  })

  it('rejeita sexo com valor não permitido', async () => {
    const schema = await buildSchema()
    expect(schema.safeParse({ ...validPayload, sexo: 'desconhecido' as 'masculino' }).success).toBe(false)
  })
})

...
export async function deleteObject(key: string): Promise<void> {
  await withCircuitBreaker(
    "s3",
    () =>
      s3.send(new DeleteObjectCommand({ Bucket: env.AWS_S3_BUCKET, Key: key })),
    { threshold: 3, resetMs: 30_000 },
  );
}
...
export async function ensureS3Lifecycle(): Promise<void> {
  ...
  try {
    ...
    if (code === "NoSuchLifecycleConfiguration") {
      ...
      logger.warn(
        "[ensureS3Lifecycle] Sem permissão para consultar lifecycle (tentando criar)",
        {
          bucket,
          rule: LIFECYCLE_RULE_ID,
          errorCode: code,
      ...
      logger.error(
        "[ensureS3Lifecycle] Erro inesperado ao consultar lifecycle (tentando criar)",
        {
          bucket,
          rule: LIFECYCLE_RULE_ID,
  ...
  try {
    ...
    logger.info(
      "[ensureS3Lifecycle] Regra criada — exames-inicio/ expira em 30 dias",
      {
        bucket,
        rule: LIFECYCLE_RULE_ID,
      },

...
async function request<T>(
  ...
  return withCircuitBreaker("asaas", async () =>
    _rawRequest<T>(method, path, body),
  );
}

async function _rawRequest<T>(
  method: string,
  path: string,
  body?: unknown,
): Promise<T> {
...
async function encontrarOuCriarCliente(
  ...
  try {
    const existing = await request<AsaasCustomerList>(
      "GET",
      `/customers?cpfCnpj=${cpfDigits}&limit=1`,
    );
    ...
    logger.info("[asaas] customer não encontrado (404), criando novo", {
      cpfPrefix: cpfDigits.slice(0, 3),
    });
  }

  if (existingId) {
    logger.info("[asaas] customer existente reutilizado", {
...
> {
  ...
  if (metodo !== "PIX") {
    if (!payment.invoiceUrl)
      throw new Error(
        `Asaas não retornou invoiceUrl para billingType=${metodo}`,
      );
    return {
      tipo: "cartao",
  ...
  const qr = await request<AsaasPixQrCode>(
    "GET",
...
export async function listarPagamentosPorReferencia(
  externalReference: string,
): Promise<AsaasPayment[]> {
  const result = await request<AsaasPaymentList>(
    "GET",
    `/payments?externalReference=${encodeURIComponent(externalReference)}&limit=5`,
  );
  return result.data;
}

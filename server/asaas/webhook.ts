...
export async function handleAsaasWebhook(req: Request, res: Response): Promise<void> {
  ...
  if (!env.ASAAS_WEBHOOK_TOKEN) {
    if (env.NODE_ENV === 'production') {
      log('ERROR', 'ASAAS_WEBHOOK_TOKEN não configurado — rejeitando webhook em produção')
      res.status(503).json({ error: 'Webhook não configurado' })
      return
    }
    log('WARN', 'ASAAS_WEBHOOK_TOKEN ausente — pulando verificação (não-produção)')
  } else {
    ...
    const paddedProvided = Buffer.concat([provided, Buffer.alloc(maxLen - provided.length)])
...
async function handlePaymentConfirmed(payment: AsaasWebhookPayment): Promise<void> {
  const { id: paymentId, externalReference, value, billingType } = payment

  log('INFO', `Payment confirmed: paymentId=${paymentId}, externalReference=${externalReference ?? 'null'}`)
  ...
  if (precadMatch) {
    ...
    if (precad.status === 'link_enviado') {
      log('WARN', `Precadastro ${precadastroId} already link_enviado — skipping (idempotent)`)
      return
    }
    ...
    if (precad.tipo === 'particular') {
      emitirNfseAsaas(paymentId).catch((err: unknown) => {
        log('WARN', `NFS-e emission failed (non-fatal): ${(err as Error).message}`)
        Sentry.captureException(err, {
  ...
  if (pacMatch) {
    ...
    if (existing.length) {
      log('WARN', `Pagamento already recorded for paymentId=${paymentId} — skipping`)
      return
    }
    ...
    log('INFO', `Pagamento inserted: pacienteId=${pacienteId}, paymentId=${paymentId}`)
    return
  }

  log('WARN', `Payment confirmed with unrecognized externalReference: ${externalReference ?? 'null'}`, { paymentId })
}

...
const retentionQueue = new Queue(RETENTION_QUEUE_NAME, {
  connection,
  prefix: QUEUE_PREFIX,
});
...
async function executarPurgaLgpd(): Promise<{
  purged: number;
  errors: number;
}> {
  ...
  if (vencidos.length === 0) {
    logger.info("[retention] nenhum registro vencido", {
      timestamp: agora.toISOString(),
    });
    return { purged: 0, errors: 0 };
  }

  logger.info("[retention] iniciando anonimização LGPD", {
  ...
  for (const row of vencidos) {
    try {
      ...
      errors++;
      logger.error("[retention] falha ao anonimizar paciente", {
        id: row.id,
        error: String(err),
      });
    }
...
export function startRetentionWorker() {
  ...
  worker.on("failed", (job, err) => {
    logger.error("[retention] job falhou", {
      jobId: job?.id,
      error: err.message,
    });
  });

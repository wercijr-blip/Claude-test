...
export async function enqueueEnviarLinkAcesso(
  ...
  if (!codigo)
    throw new Error(
      "enqueueEnviarLinkAcesso: codigo vazio — abortando para não enviar link inacessível",
    );
  if (!link.includes("/acesso/"))
    throw new Error(
      `enqueueEnviarLinkAcesso: link sem token de acesso — link="${link}"`,
    );
...
export function startLinkAcessoWorker() {
  const worker = new Worker(
    ...
    async (job) => {
      ...
      if (!codigo)
        throw new Error(
          `[linkAcessoWorker] job ${job.id}: codigo vazio — não enviar`,
        );
      if (!link.includes("/acesso/"))
        throw new Error(
          `[linkAcessoWorker] job ${job.id}: link sem token — link="${link}"`,
        );
      ...
      if (telefone) {
        ...
        await enviarWhatsApp(telefone, msg).catch((e: unknown) =>
          logger.warn("[linkAcessoQueue] notificação falhou", {
            error: String(e),
          }),
        );
      }
  ...
  worker.on("failed", (job, err) => {
    logger.error(
      `[linkAcessoQueue] Job ${job?.id} falhou (${job?.attemptsMade} tentativas)`,
      { message: err.message },
    );

...
import { PESQUISA_QUEUE_NAME, QUEUE_PREFIX, connection, PESQUISA_WORKER_OPTS, persistDlq } from './queues.ts'
...
export function startPesquisaWorker() {
  const worker = new Worker(
    ...
    async (job) => {
      ...
      if (email) {
        await enviarPesquisaSatisfacao(email, nome, link).catch((e: unknown) => logger.warn('[pesquisaQueue] notificação falhou', { error: String(e) }))
      }
      ...
      if (telefone) {
        ...
        await enviarWhatsApp(telefone, msg).catch((e: unknown) => logger.warn('[pesquisaQueue] notificação falhou', { error: String(e) }))
      }
    },

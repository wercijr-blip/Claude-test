...
async function assinarESalvar(
  ...
  await db
    .insert(pdfs)
    .values({ pacienteId, s3Key: key, tipo, certificadoSerial, assinadoEm });
  return signed;
}
...
export function startPdfWorker() {
  const worker = new Worker(
    ...
    async (job) => {
      ...
      const [prescricaoExistente] = await db
        ...
        .from(pdfs)
        .where(
          and(eq(pdfs.pacienteId, pacienteId), eq(pdfs.tipo, "prescricao")),
        )
        .limit(1);
      if (prescricaoExistente) {
        logger.info("[pdfQueue] PDFs já existentes, pulando geração (retry)", {
          pacienteId,
        });
      ...
      const telefone = p.telefoneEncrypted
        ? decrypt(p.telefoneEncrypted)
        : null;
      ...
      const signedPresc = await assinarESalvar(
        pacienteId,
        prescBuf,
        "Receita PrEP — Facilita PrEP",
        "prescricao",
        "prescricao.pdf",
      );
      ...
      if (tipoConsulta === "primeiro_atendimento") {
        const cadastroBuf = Buffer.from(
          await preencherCadastroSUS({
            pacienteId,
            cpf,
            nome,
            nomeMae: nomeMae ?? "",
        ...
        const signedCad = await assinarESalvar(
          pacienteId,
          cadastroBuf,
          "Cadastro de Usuário SUS PrEP — Facilita PrEP",
          "cadastro",
          "cadastro-sus.pdf",
        );
        gerados.push({
          filename: "cadastro-usuario-sus-prep.pdf",
          buffer: signedCad,
        });
      }
      ...
      const signedFicha = await assinarESalvar(
        pacienteId,
        fichaBuf,
        "Ficha de Atendimento PrEP — Facilita PrEP",
        "ficha_atendimento",
        "ficha-atendimento.pdf",
      );
      gerados.push({
        filename: "ficha-atendimento-prep.pdf",
        buffer: signedFicha,
      });

      // 4. Pedidos de exame (sempre)
      try {
        const tipoConsultaParaPedidos = (tipoConsulta ??
          "primeiro_atendimento") as "primeiro_atendimento" | "ja_faco_prep";
        ...
        const pedidosBuffers = [
          {
            ...
            filename: "pedido-exames-completo.pdf",
            titulo: "Pedido de Exames Completo — Facilita PrEP",
          },
          {
            buf: ist,
            tipo: "pedido_ist",
            filename: "pedido-sorologicos-ist.pdf",
            titulo: "Pedido de Sorologias IST — Facilita PrEP",
          },
          {
            buf: hiv,
            tipo: "pedido_hiv",
            filename: "pedido-anti-hiv.pdf",
            titulo: "Pedido de Anti-HIV — Facilita PrEP",
          },
          {
            buf: densitometria,
            tipo: "pedido_densitometria",
            filename: "pedido-densitometria-ossea.pdf",
            titulo: "Pedido de Densitometria Óssea — Facilita PrEP",
          },
        ] as const;
        for (const { buf, tipo, filename, titulo } of pedidosBuffers) {
          try {
            const signedBuf = await assinarESalvar(
              pacienteId,
              buf,
              titulo,
              tipo,
        ...
        logger.error(
          "[pdfQueue] Falha ao gerar pedidos de exame (continuando)",
      ...
      const signedOri = await assinarESalvar(
        pacienteId,
        orientacaoBuf,
        "Documento de Orientação PrEP — Facilita PrEP",
        "orientacao",
        "orientacao.pdf",
      );
      ...
      if (emailAddr) {
        await enviarPrescricaoPronta(emailAddr, nome, gerados).catch(
          (e: unknown) =>
            logger.warn("[pdfQueue] notificação falhou", { error: String(e) }),
        );
      }
  ...
  worker.on("failed", (job, err) => {
    logger.error(
      `[pdfQueue] Job ${job?.id} falhou (${job?.attemptsMade} tentativas)`,
      { message: err.message },
    );

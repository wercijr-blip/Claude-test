...
async function executarDrMensal() {
  ...
  logger[level]("[dr-mensal] resultado", {
    checks,
    allOk,
...
export function startLembreteWorker() {
  const worker = new Worker(
    ...
    async (job) => {
      ...
      for (const p of pendentes) {
        ...
        if (
          p.ultimoLembreteAt &&
          agora.getTime() - p.ultimoLembreteAt.getTime() < 20 * 60 * 60 * 1000
        )
          continue;

        const diasRestantes = Math.ceil(
          (p.linkExpiresAt!.getTime() - agora.getTime()) /
            (24 * 60 * 60 * 1000),
        );
        ...
        const nome = p.precadNome
          ? decrypt(p.precadNome).split(" ")[0]
          : "Paciente";
        ...
        try {
          ...
          logger.warn("[lembreteQueue] falha ao regenerar token", {
            tokenId: p.tokenId,
            error: String(e),
          });
          continue;
        ...
        await enviarLinkAcessoIntake(
          ...
          logger.warn("[lembreteQueue] notificação falhou", {
            error: String(e),
          }),
        );
  ...
  worker.on("failed", (job, err) => {
    logger.error(`[lembreteQueue] Job ${job?.id} falhou`, {
      message: err.message,
    });

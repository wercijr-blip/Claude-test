...
export async function buscarReferenciasPorQuery(
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn("[zotero] buscarReferenciasPorQuery falhou", {
        status: res.status,
        query,
      });
    ...
    logger.warn("[zotero] buscarReferenciasPorQuery erro", {
      query,
      err: String(err),
    });
    zoteroCircuit.recordFailure();
...
export async function buscarReferenciasPorTag(
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn("[zotero] buscarReferenciasPorTag falhou", {
        status: res.status,
        tag,
      });
      zoteroCircuit.recordFailure();
    ...
    logger.warn("[zotero] buscarReferenciasPorTag erro", {
      tag,
      err: String(err),
    });
    zoteroCircuit.recordFailure();
...
export async function salvarArtigoPubMed(
  ...
  if (opcoes?.cid10)
    tags.push({ tag: opcoes.cid10.replace(".", "").toUpperCase() });
  ...
  try {
    ...
    if (!res.ok) {
      logger.warn("[zotero] salvarArtigoPubMed falhou", {
        status: res.status,
        pmid: artigo.pmid,
      });
      zoteroCircuit.recordFailure();
    ...
    logger.info("[zotero] Artigo salvo", {
      pmid: artigo.pmid,
      titulo: artigo.titulo.slice(0, 60),
    });
    zoteroCircuit.recordSuccess();
    return true;
  } catch (err) {
    logger.warn("[zotero] salvarArtigoPubMed erro", {
      pmid: artigo.pmid,
      err: String(err),
    });
    zoteroCircuit.recordFailure();
...
export function formatarZoteroParaPrompt(items: ZoteroItem[]): string {
  if (items.length === 0)
    return "Nenhuma referência encontrada na biblioteca Zotero.";

  return items
    .map((item, i) => {
      const autores =
        item.autores.length > 0
          ? item.autores.slice(0, 3).join(", ") +
            (item.autores.length > 3 ? " et al." : "")

...
async function checkMinuteLimit(): Promise<void> {
  ...
  try {
    ...
    if (count > MAX_MINUTE_LLM_CALLS) {
      ...
      limitExceeded = true;
      logger.warn(
        "[llm] limite por minuto atingido — job aguardará próximo ciclo",
        {
          count: count - 1,
      ...
      throw new Error(
        `Limite por minuto de análises por IA atingido (${MAX_MINUTE_LLM_CALLS}/min). Job será reprocessado.`,
      );
    }
  } catch (err) {
    if (limitExceeded) throw err;
    logger.warn("[llm] Redis indisponível — ignorando limite por minuto", {
      error: String(err),
    });
  }
}
...
async function checkDailyLimit(): Promise<void> {
  ...
  try {
    ...
    if (count > MAX_DAILY_LLM_CALLS) {
      ...
      limitExceeded = true;
      logger.warn("[llm] limite diário atingido", {
        count: count - 1,
        limit: MAX_DAILY_LLM_CALLS,
        key,
      });
      throw new Error(
        `Limite diário de análises por IA atingido (${MAX_DAILY_LLM_CALLS}/dia). Tente novamente amanhã.`,
      );
    }
    const alertThreshold = Math.floor(
      MAX_DAILY_LLM_CALLS * LLM_ALERT_THRESHOLD,
    );
    if (count === alertThreshold) {
      ...
      if (!alreadyAlerted) {
        ...
        logger.warn(
          `[llm] ${percentual}% do limite diário atingido — alerta enviado`,
          { count, limit: MAX_DAILY_LLM_CALLS },
        );
      }
    }
  } catch (err) {
    if (limitExceeded) throw err;
    logger.warn("[llm] Redis indisponível — ignorando limite diário", {
      error: String(err),
    });
  }
}
...
export function parseIaResponse(
  ...
  try {
    ...
    logger.warn(
      "[examAnalysis] resposta da IA não parseável — fallback nao_identificado",
      {
        exameId,
        sessionId,
...
export async function analisarExame(exameId: number): Promise<ResultadoIa> {
  if (!isEnabled("EXAM_AI_ANALYSIS")) {
    throw new Error(
      "Análise por IA desativada — encaminhar para revisão manual do médico",
    );
  }
  ...
  const qualidade = validateExameQuality(
    exame.s3Key,
    exame.tipoExame,
    exame.tamanhoBytes,
  );
  if (!qualidade.valid) {
    logger.warn(
      "[examAnalysis] exame rejeitado por qualidade insuficiente — BPIA.03",
      {
        exameId,
    ...
    throw new Error(
      `Dados insuficientes para análise por IA: ${qualidade.reason}`,
    );
  }
  if (qualidade.warning) {
    logger.warn("[examAnalysis] aviso de qualidade de dados", {
      exameId,
      aviso: qualidade.warning,
    });
  }
  ...
  const textFieldsToCheck = [exame.nomeArquivo, exame.tipoExame].filter(
    Boolean,
  ) as string[];
  for (const field of textFieldsToCheck) {
    if (detectPromptInjection(field)) {
      logger.warn(
        "[examAnalysis] possível injeção de prompt detectada — NGS1.12",
        { exameId, field },
      );
      throw new Error(
        "Entrada rejeitada: padrão adversarial detectado no exame (NGS1.12)",
      );
    }
  }
  ...
  try {
    ...
    throw new Error(
      `Falha na requisição à API de IA: ${(fetchErr as Error).message}`,
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Erro na análise por IA (HTTP ${response.status}): ${body}`,
    );
  }
  ...
  const resultado = parseIaResponse(
    text,
    sessionId,
    exame.tipoExame ?? "outro",
    exameId,
  );
  ...
  if (
    ...
    logger.warn(
      "[examAnalysis] resultado anômalo — RT notificado via audit log — NGS1.10",
      {
        exameId,
        sessionId,
        motivo,

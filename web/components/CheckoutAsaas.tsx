...
export default function CheckoutAsaas({ paymentId, pixQrCode, pixCopiaECola }: Props) {
  ...
  useEffect(() => {
    ...
    if ((status === 'RECEIVED' || status === 'CONFIRMED') && !confirmedRef.current) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 max-w-md w-full text-center">
          ...
          <p className="text-slate-400 text-xs">Verifique também sua caixa de spam.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 max-w-md w-full">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">Escaneie o QR Code ou copie o código</p>
        </div>
        ...
        <div className="bg-slate-50 rounded-xl p-3 mb-4">
          ...
          <p className="text-xs text-slate-700 break-all font-mono leading-relaxed">
            {pixCopiaECola.length > 80 ? `${pixCopiaECola.slice(0, 80)}…` : pixCopiaECola}
          </p>
        </div>

...
export const clinicalSessions = mysqlTable(
  "clinical_sessions",
  {
    id: int("id").primaryKey().autoincrement(),
...
export const soapNotes = mysqlTable(
  "soap_notes",
  {
    id: int("id").primaryKey().autoincrement(),
    ...
    retencaoAte: datetime("retencao_ate")
      .notNull()
      .default(sql`DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 20 YEAR)`),
    deletedAt: datetime("deleted_at"),
    createdAt: datetime("created_at")
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
...
export const conductAlerts = mysqlTable(
  "conduct_alerts",
  {
    id: int("id").primaryKey().autoincrement(),
    soapNoteId: int("soap_note_id")
      .notNull()
...
export const publicationDrafts = mysqlTable(
  "publication_drafts",
  {
    id: int("id").primaryKey().autoincrement(),
    medicoId: int("medico_id")
    ...
    createdAt: datetime("created_at")
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
    atualizadoEm: datetime("atualizado_em")
      .notNull()
      .default(sql`CURRENT_TIMESTAMP`),
  },
  (t) => ({
    ...
    unicoCid10: uniqueIndex("idx_pdrafts_uniq_cid10").on(
      t.medicoId,
      t.tipo,
      t.cid10,
      t.status,
...
export const clinicalDigests = mysqlTable(
  "clinical_digests",
  {
    id: int("id").primaryKey().autoincrement(),
    medicoId: int("medico_id")

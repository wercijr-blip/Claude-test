...
const features = [
  { icon: '🎤', title: 'Transcrição em Tempo Real', desc: 'Áudio da consulta transcrito automaticamente com reconhecimento de terminologia médica especializada.' },
  { icon: '📋', title: 'SOAP Automático', desc: 'Geração inteligente de prontuário estruturado nos campos S, O, A e P ao fim de cada consulta.' },
  { icon: '🧠', title: 'Base de Conhecimento', desc: 'Tópicos clínicos extraídos de cada consulta e organizados automaticamente para estudo da equipe.' },
  { icon: '📊', title: 'Dashboard Administrativo', desc: 'Visão completa da clínica: produção por médico, diagnósticos frequentes e indicadores clínicos.' },
]
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setLocation('/login')} className="text-sm text-slate-600 hover:text-teal-700 transition-colors hidden sm:block">Entrar</button>
            <button onClick={() => setLocation('/login')} className="bg-teal-700 hover:bg-teal-800 text-white text-sm font-medium px-4 py-2 rounded-xl transition-colors">Agendar Demo</button>
          </div>
        </div>
      </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-12">
              <button onClick={() => setLocation('/login')} className="bg-teal-700 hover:bg-teal-800 text-white font-medium px-6 py-3 rounded-xl transition-colors text-sm">Agendar demonstração gratuita</button>
              <button onClick={() => setLocation('/login')} className="border border-slate-200 text-slate-700 hover:border-teal-300 hover:text-teal-700 font-medium px-6 py-3 rounded-xl transition-colors text-sm">Já tenho conta →</button>
            </div>
            <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-lg max-w-sm">
              <p className="text-slate-700 text-sm leading-relaxed mb-4 italic">“Economizo 40 minutos por dia em documentação. Esse tempo voltou para os meus pacientes.”</p>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-sm">CO</div>
                <div>
      ...
      <section id="depoimentos" className="max-w-6xl mx-auto px-6 py-24">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <div key={i} className={`rounded-2xl p-6 border ${i === 1 ? 'bg-teal-700 border-teal-700 text-white' : 'bg-white border-slate-100'}`}>
              <div className="text-3xl mb-4 opacity-40">“</div>
              <p className={`text-sm leading-relaxed mb-6 ${i === 1 ? 'text-teal-100' : 'text-slate-600'}`}>{t.text}</p>
            ...
            {faqs.map((faq, i) => (
              <div key={i} className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
                <button className="w-full flex items-center justify-between px-6 py-5 text-left" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  ...
                  <span className={`text-teal-600 text-lg transition-transform duration-200 ${openFaq === i ? 'rotate-45' : ''}`}>+</span>
                </button>
                {openFaq === i && (
                  <div className="px-6 pb-5"><p className="text-slate-500 text-sm leading-relaxed">{faq.a}</p></div>
                )}
              </div>
            ))}
          </div>

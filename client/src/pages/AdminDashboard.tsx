  ...
  return (
    <DashboardLayout>
      <div className="p-6 space-y-8">
              ...
              <tbody className="divide-y divide-slate-50">
                {byDoctor.data?.map((d) => (
                  <tr key={d.userId} className="hover:bg-slate-50">
                    ...
                    <td className="py-3">
                      <button
                        onClick={() => setSelectedDoctor(d.userId)}
        ...
        {/* Seção 5 — Métricas da plataforma */}
        <section className="bg-white rounded-2xl border border-slate-200 p-6">
          <h2 className="text-base font-semibold text-slate-700 mb-4">Métricas da Plataforma</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <StatCard label="Min. transcritos" value={platform.data?.totalAudioMinutes ?? '—'} icon="🎙️" />
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    <Pie data={platform.data.qualityDist} dataKey="count" nameKey="bucket" cx="50%" cy="50%" outerRadius={70} innerRadius={40}>
                      {platform.data.qualityDist.map((_, i) => (
        ...
        {clinical.data && (
          <section className="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <p className="text-sm font-medium text-slate-600 mb-3">Top Exames Solicitados</p>
                <div className="space-y-2">
                  {clinical.data.topExams.slice(0, 8).map((e, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <span className="text-xs text-slate-600 w-36 truncate">{e.name}</span>
                      <div className="flex-1 bg-slate-100 rounded-full h-1.5">
                        <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${e.percentage}%` }} />
                      </div>
                      <span className="text-xs text-slate-400 w-8 text-right">{e.count}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-slate-600 mb-3">Top Medicamentos</p>
                <div className="space-y-2">
                  {clinical.data.topMeds.slice(0, 8).map((m, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <span className="text-xs text-slate-600 w-32 truncate">{m.name}</span>
                      <span className="text-xs text-slate-400 bg-slate-100 rounded px-1">{m.predominantRoute}</span>
                      <span className="text-xs text-slate-500 ml-auto">{m.count}</span>
                    </div>
                  ))}
                </div>
              </div>

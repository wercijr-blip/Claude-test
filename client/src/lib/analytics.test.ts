...
import { persistUtms, getStoredUtms, generateEventId, trackPageView, trackPurchase, initScrollDepth } from './analytics.ts'
...
describe('persistUtms', () => {
  ...
  it('does not overwrite stored UTMs when URL has no UTMs', () => {
    sessionStorageMock.setItem('_fp_utms', JSON.stringify({ utm_source: 'facebook' }))
...
describe('trackPurchase', () => {
  ...
  it('includes UTMs when they are stored', () => {
    sessionStorageMock.setItem('_fp_utms', JSON.stringify({ utm_source: 'meta' }))
  ...
  it('pushes enhanced_conversion_data when email is provided', () => {
    ...
    const ecEvt = window.dataLayer.find(e => e.event === 'enhanced_conversion_data')
    expect(ecEvt?.enhanced_conversion_data).toMatchObject({ email: 'patient@test.com' })
  })
})
...
describe('initScrollDepth', () => {
  afterEach(() => { window.__fp_scroll_attached__ = false })

...
const data = {
    ...
    items: [
      {
        ...
        palavras: ['PrEP online Brasília', 'consulta PrEP particular', 'PrEP teleconsulta', 'prescrição PrEP digital', 'PrEP sem SUS'],
      ...
      },
      {
        ...
        palavras: ['remarketing de visitantes', 'lookalike de leads convertidos'],
      ...
      },
      {
        ...
        palavras: ['Homens 20-45 interessados em saúde', 'LGBT+', 'Saúde sexual'],
      ...
      },
      {
        ...
        palavras: ['Remarketing: visitou o site mas não converteu', 'Lead Ads para captura de contato'],
    ...
    blocks: [
      {
        ...
        sugerido: 'Sua proteção contra o HIV, do jeito certo — online, rápido e com médico de verdade',
        tipo: 'Hero Headline',
      },
      {
      ...
      },
      {
        ...
        sugerido: '"Recebi minha receita em 24h. Processo simples e médico muito atencioso." — Paciente via Facilita PrEP',
        tipo: 'Depoimento',
      },
      {
        atual: '(provavelmente ausente)',
        problema: 'Falta FAQs para objeções comuns',
        sugerido: 'Por que não usar o SUS? É realmente privado? Meus dados ficam seguros? Funciona no meu estado?',
        tipo: 'FAQ / Objeções',
      },
    ],
  },
  ...
  confianca: {
    ...
    items: [
      { icon: '⚕️', title: 'CRM e RQE visíveis', desc: 'Nome do médico responsável, CRM-DF 16381 e RQE 14486 devem aparecer no footer e na página "Quem somos". Isso é exigência legal e gera confiança.' },
      { icon: '🔒', title: 'Selos de segurança', desc: 'ICP-Brasil, LGPD compliance, SSL visível. Adicionar seção "Seus dados estão seguros" com explicação simples sobre criptografia e privacidade.' },
      { icon: '📋', title: 'CFM - Telemedicina', desc: 'Referenciar conformidade com Resolução CFM 2.314/2022 na página de teleconsulta. Isso diferencia de plataformas ilegais ou gray-market.' },
      { icon: '🏥', title: 'ATOS e IASO como âncoras institucionais', desc: 'Mencionar que a plataforma é operada pelo mesmo médico RT das clínicas ATOS Saúde e IASO Saúde amplia a percepção de solidez.' },
      { icon: '📱', title: 'WhatsApp de suporte visível', desc: 'Para tráfego pago, um botão flutuante do WhatsApp aumenta conversão em 15-30%. Pacientes com dúvidas sobre PrEP preferem conversar antes de agendar.' },
      { icon: '⭐', title: 'Social proof quantificado', desc: 'Ex: "Mais de 200 pacientes em PrEP acompanhados" ou "Prescrições emitidas em todos os estados". Números concretos vencem claims genéricos.' },
    ],
  },
  acoes: {
...
function SectionHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <h2 style={{ margin: '0 0 4px', color: '#ccd6f6', fontSize: 20, fontWeight: 700 }}>{title}</h2>
...
export default function AuditoriaPage() {
  ...
  return (
    ...
    <div style={{ fontFamily: "'Georgia', serif", background: '#0f1117', minHeight: '100vh', color: '#e8e8e8' }}>
      ...
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, #1a1f35 0%, #0d1528 100%)', borderBottom: '1px solid #2a3050', padding: '32px 24px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#c0392b' }} />
            <span style={{ fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', color: '#8892b0', fontFamily: 'monospace' }}>
              AUDITORIA · FACILITA PREP
            </span>
          </div>
          <h1 style={{ fontSize: 28, fontWeight: 700, margin: '0 0 8px', color: '#ccd6f6', letterSpacing: -0.5 }}>
            Relatório de Otimização para Tráfego Pago
          </h1>
          <p style={{ margin: 0, color: '#8892b0', fontSize: 14, lineHeight: 1.6 }}>
      ...
      {/* Nav */}
      <div style={{ background: '#13172a', borderBottom: '1px solid #1e2540', overflowX: 'auto' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', gap: 2, padding: '0 24px' }}>
          {sections.map((s) => (
            <button
              key={s.id}
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {active === 'diagnostico' && (
          <div>
            <SectionHeader title={data.diagnostico.title} subtitle={data.diagnostico.subtitle} />
              ...
              {data.diagnostico.items.map((item, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
                    ...
                    <h3 style={{ margin: 0, fontSize: 15, color: '#ccd6f6', fontWeight: 600 }}>{item.title}</h3>
                  </div>
                  <p style={{ margin: '0 0 12px', color: '#8892b0', fontSize: 13, lineHeight: 1.7 }}>{item.desc}</p>
                  <div style={{ background: '#0d1528', borderRadius: 6, padding: '10px 14px' }}>
                    <span style={{ color: '#27ae60', fontSize: 11, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: 1 }}>✓ solução </span>
                    <span style={{ color: '#a8b2d8', fontSize: 13 }}>{item.fix}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {active === 'trafego' && (
          <div>
            <SectionHeader title={data.trafego.title} subtitle={data.trafego.subtitle} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {data.trafego.items.map((item, i) => (
                <div key={i} style={{ background: '#13172a', border: '1px solid #1e2540', borderRadius: 10, overflow: 'hidden' }}>
                  <div style={{
                    background: item.plataforma === 'Google Ads' ? '#1a3a6b' : '#1a2f5a',
                    padding: '12px 20px',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 12,
                  }}>
                    <span style={{ fontSize: 18 }}>{item.plataforma === 'Google Ads' ? '🔵' : '🟣'}</span>
                    <div>
                      <span style={{ color: '#ccd6f6', fontWeight: 700, fontSize: 14 }}>{item.plataforma}</span>
                      <span style={{ color: '#8892b0', fontSize: 12, marginLeft: 10 }}>— {item.tipo}</span>
                    </div>
                  </div>
                  <div style={{ padding: 20 }}>
                    <div style={{ background: '#0d1528', borderRadius: 8, padding: 14, marginBottom: 14 }}>
                      <div style={{ fontSize: 11, color: '#64ffda', fontFamily: 'monospace', marginBottom: 4 }}>HEADLINE</div>
                      <div style={{ fontSize: 15, color: '#e8e8e8', fontWeight: 600 }}>"{item.headline}"</div>
                    </div>
                    <p style={{ color: '#8892b0', fontSize: 13, lineHeight: 1.7, marginBottom: 14 }}>{item.desc}</p>
                    <div>
                      <div style={{ fontSize: 11, color: '#8892b0', fontFamily: 'monospace', marginBottom: 8 }}>PALAVRAS-CHAVE / AUDIÊNCIAS</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {item.palavras.map((p, j) => (
                          <span key={j} style={{ background: '#1e2540', border: '1px solid #2a3558', color: '#a8b2d8', padding: '4px 10px', borderRadius: 20, fontSize: 12 }}>{p}</span>
                        ))}
                      </div>
                    </div>
                    {item.nota && (
                      <div style={{ marginTop: 12, padding: '8px 12px', background: '#1a1f35', borderRadius: 6, borderLeft: '3px solid #f39c12' }}>
                        <span style={{ color: '#f39c12', fontSize: 11, fontFamily: 'monospace' }}>⚠ nota: </span>
                        <span style={{ color: '#8892b0', fontSize: 12 }}>{item.nota}</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        {active === 'copy' && (
          <div>
            <SectionHeader title={data.copy.title} subtitle={data.copy.subtitle} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {data.copy.blocks.map((block, i) => (
                <div key={i} style={{ background: '#13172a', border: '1px solid #1e2540', borderRadius: 10, overflow: 'hidden' }}>
                  <div style={{ background: '#1e2540', padding: '8px 16px' }}>
                    <span style={{ fontSize: 11, color: '#64ffda', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: 1 }}>{block.tipo}</span>
                  </div>
                  <div style={{ padding: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <div>
                      <div style={{ fontSize: 10, color: '#e74c3c', fontFamily: 'monospace', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1 }}>✗ Atual</div>
                      <div style={{ background: '#0d1528', borderRadius: 6, padding: 12, color: '#8892b0', fontSize: 13, fontStyle: 'italic', borderLeft: '3px solid #e74c3c33' }}>{block.atual}</div>
                      <div style={{ marginTop: 8, fontSize: 11, color: '#e74c3c' }}>{block.problema}</div>
                    </div>
                    <div>
                      <div style={{ fontSize: 10, color: '#27ae60', fontFamily: 'monospace', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1 }}>✓ Sugerido</div>
                      <div style={{ background: '#0d2818', borderRadius: 6, padding: 12, color: '#ccd6f6', fontSize: 13, fontWeight: 500, borderLeft: '3px solid #27ae60' }}>{block.sugerido}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {active === 'seo' && (
          <div>
            <SectionHeader title={data.seo.title} subtitle={data.seo.subtitle} />
            <div style={{ background: '#13172a', border: '1px solid #1e2540', borderRadius: 10, padding: 24 }}>
                ...
                {data.seo.checklist.map((item, i) => (
                  ...
                  >
                    <div style={{
                      ...
                      background: checkedItems[i] ? '#27ae60' : 'transparent',
                      display: 'flex',
                      alignItems: 'center',
                      ...
                      {checkedItems[i] && <span style={{ color: 'white', fontSize: 12 }}>✓</span>}
                    </div>
                    <span style={{
              ...
              <div style={{ marginTop: 16, textAlign: 'right', color: '#8892b0', fontSize: 12 }}>
                {Object.values(checkedItems).filter(Boolean).length} / {data.seo.checklist.length} concluídos
              </div>
            </div>
          </div>
        )}
        ...
        {active === 'confianca' && (
          <div>
            <SectionHeader title={data.confianca.title} subtitle={data.confianca.subtitle} />
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {data.confianca.items.map((item, i) => (
                <div key={i} style={{ background: '#13172a', border: '1px solid #1e2540', borderRadius: 10, padding: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <span style={{ fontSize: 28 }}>{item.icon}</span>
                  <h3 style={{ margin: 0, color: '#ccd6f6', fontSize: 14, fontWeight: 700 }}>{item.title}</h3>
                  <p style={{ margin: 0, color: '#8892b0', fontSize: 12, lineHeight: 1.7 }}>{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {active === 'acoes' && (
          <div>
            <SectionHeader title={data.acoes.title} subtitle={data.acoes.subtitle} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {data.acoes.fases.map((fase, i) => (
                <div key={i} style={{ background: '#13172a', border: '1px solid #1e2540', borderLeft: `4px solid ${fase.cor}`, borderRadius: 10, padding: 20 }}>
                  <div style={{ color: fase.cor, fontFamily: 'monospace', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ background: fase.cor, color: 'white', borderRadius: '50%', width: 22, height: 22, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700 }}>{i + 1}</span>
                    {fase.fase}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {fase.acoes.map((acao, j) => (
                      <div key={j} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                        <span style={{ color: fase.cor, fontSize: 12, marginTop: 2, flexShrink: 0 }}>→</span>
                        <span style={{ color: '#a8b2d8', fontSize: 13, lineHeight: 1.6 }}>{acao}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* Budget recomendado */}
            <div style={{ marginTop: 24, background: '#13172a', border: '1px solid #27ae6044', borderRadius: 10, padding: 20 }}>
              <h3 style={{ color: '#27ae60', margin: '0 0 16px', fontSize: 14, fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: 1 }}>
                💰 Budget Inicial Recomendado (Mês 1)
              </h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {[
                  { label: 'Google Search', valor: 'R$ 1.500/mês', nota: 'Alta intenção' },
                  { label: 'Meta Ads', valor: 'R$ 600/mês', nota: 'Awareness + remarketing' },
                  { label: 'Total inicial', valor: 'R$ 2.100/mês', nota: '~R$ 70/dia' },
                ].map((b, i) => (
                  <div key={i} style={{ background: '#0d1528', borderRadius: 8, padding: 14, textAlign: 'center' }}>
                    <div style={{ color: '#8892b0', fontSize: 11, marginBottom: 6 }}>{b.label}</div>
                    <div style={{ color: '#64ffda', fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{b.valor}</div>
                    <div style={{ color: '#8892b0', fontSize: 11 }}>{b.nota}</div>
                  </div>
                ))}
              </div>
              <p style={{ margin: '14px 0 0', color: '#8892b0', fontSize: 12, lineHeight: 1.6 }}>
                <strong style={{ color: '#a8b2d8' }}>Meta de CPL (Custo por Lead):</strong> R$ 40–80 no Google Search | R$ 15–40 no Meta (leads frios).
                Com ticket médio de consulta entre R$ 150–250, break-even a partir de 1 conversão a cada 2–3 leads qualificados.
              </p>
            </div>
          </div>
        )}
      ...
      {/* Footer */}
      <div style={{ borderTop: '1px solid #1e2540', padding: '20px 24px', textAlign: 'center' }}>
        <p style={{ margin: 0, color: '#4a5568', fontSize: 11, fontFamily: 'monospace' }}>

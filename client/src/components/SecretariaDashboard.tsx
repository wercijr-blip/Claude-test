...
function PlanosTab() {
  ...
  const aprovar = trpc.intake.aprovar.useMutation({
    onSuccess: () => refetch(),
  });
  const rejeitar = trpc.intake.rejeitar.useMutation({
    onSuccess: () => refetch(),
  });
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-800">
            Planos de saúde — aguardando validação
          </h2>
          <p className="text-sm text-slate-500 mt-0.5">
            Verifique os documentos e aprove ou rejeite o cadastro.
          </p>
        </div>
        <span className="bg-amber-100 text-amber-700 text-xs font-semibold px-3 py-1 rounded-full">
          {pendentes?.length ?? 0} pendente
          {(pendentes?.length ?? 0) !== 1 ? "s" : ""}
        </span>
      </div>
      ...
      {!pendentes?.length && (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-slate-600 font-medium">
            Nenhum plano aguardando validação
          </p>
            ...
            {expandedId === p.id && (
              <div className="border-t border-slate-100 p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() =>
                      p.carteirinhaS3Key && abrirDocumento(p.carteirinhaS3Key)
                    }
                  ...
                  <button
                    onClick={() =>
                      p.documentoS3Key && abrirDocumento(p.documentoS3Key)
                    }
...
export default function SecretariaDashboard() {
  ...
  const { data: documentos } = trpc.secretaria.listarDocumentos.useQuery({
    status: statusFiltro,
  });
  ...
  const revogar = trpc.token.revogar.useMutation({
    onSuccess: () => refetch(),
  });
  ...
  const linkAcesso = novoToken
    ? `${window.location.origin}/acesso/${novoToken}`
    : null;
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-blue-700">
            Dashboard Secretaria — Facilita PrEP
          </h1>
          <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex gap-3">
                  {(["privado", "convenio"] as const).map((t) => (
                    <label
                      key={t}
                      className={`flex items-center gap-2 border rounded-lg px-4 py-2 cursor-pointer text-sm transition-colors ${tipo === t ? "border-blue-500 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600"}`}
                    >
                      <input
                        type="radio"
                        value={t}
            ...
            {linkAcesso && (
              <div className="mt-4 bg-green-50 border border-green-200 rounded-xl p-4">
                ...
                <p className="text-xs text-green-600 mt-2">
                  Válido por 7 dias. Envie ao paciente por e-mail ou WhatsApp.
                </p>
              </div>
            )}
          </div>
                  ...
                  {!t.revokedAt && (
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        title={
                          reenviadoId === t.id ? "Enviado!" : "Reenviar link"
                        }
                      ...
                      <button
                        onClick={() => revogar.mutate({ tokenId: t.id })}
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[iaStatus] ?? "bg-slate-100 text-slate-500"}`}
                          >
                            {iaStatus}
                          </span>
                        </td>

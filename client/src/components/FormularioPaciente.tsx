  ...
  const { data: pdfs, isLoading } = trpc.paciente.downloadPdfs.useQuery(
    { pacienteId },
    {
      refetchInterval: (query) =>
        query.state.data && query.state.data.length > 0 ? false : 4000,
  ...
  return (
    <div className="min-h-screen bg-warm-bg flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-8 max-w-lg w-full">
        ...
        <h2 className="font-display text-3xl font-light text-fp-dark text-center mb-1">
          Pronto! Seus documentos estão prontos
        </h2>
        <p className="text-slate-500 text-sm text-center mb-6">
          Obrigado por confiar no Facilita PrEP. Em instantes você receberá tudo
          no seu e-mail — assinado digitalmente com certificado ICP-Brasil.
        </p>
          ...
          <div className="space-y-3">
            ...
            <div className="mt-6 border-t border-slate-100 pt-6 space-y-4">
              ...
              <div className="bg-brand-pale border border-brand-light rounded-xl p-4">
                <p className="text-brand-dark text-xs font-semibold mb-1">
                ...
                <p className="text-brand text-xs leading-relaxed">
                  ...
                  do Brasil. A assinatura ICP-Brasil tem validade jurídica em
              ...
              <div className="bg-sage-pale border border-sage-light rounded-xl p-4">
                <p className="text-sage-dark text-xs font-semibold mb-3">
                  🏥 Rede pública — o que levar à UDM
                </p>

                <div className="space-y-3">
                  <div>
                    <p className="text-xs font-semibold text-sage-dark mb-1.5">
                      Se é sua primeira retirada:
                    </p>
...
export default function FormularioPaciente({
  ...
  const { data: intakeData, isLoading: intakeLoading } =
    trpc.paciente.dadosIntake.useQuery(undefined, { retry: false });
  const { data: consultaStatus } = trpc.consulta.status.useQuery(undefined, {
    retry: false,
  });
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Etapa {currentStep} de {TOTAL_FORM_STEPS} —{" "}
            {FORM_STEPS[currentStep - 1]?.titulo}
          </p>
            ...
            {currentStep === 3 && (
              <StepContato
                ...
                defaultValues={{
                  ...
                  telefone: intakeData?.telefone ?? undefined,
                }}
              />
            )}
            {currentStep === 4 && (
              <StepConduta
                {...stepProps}
                examData={{
                  dataExame: (consultaStatus as { dataExame?: string | null })
                    ?.dataExame,
                  resultadoHiv: (
                    consultaStatus as { resultadoHiv?: string | null }
                  )?.resultadoHiv,
                }}
                tipoConsulta={
                  (

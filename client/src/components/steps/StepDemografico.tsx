...
export default function StepDemografico({ pacienteId, onNext, onBack }: Props) {
  ...
  const salvar = trpc.paciente.salvarStep2.useMutation({
    onSuccess: () => {
      clearDraft();
      onNext();
    },
      ...
      >
        <Field label="Cor/Raça" error={errors.corRaca?.message} name="corRaca">
          <select
            id="corRaca"
            {...register("corRaca")}
            className={inputCls(!!errors.corRaca)}
            aria-invalid={errors.corRaca ? true : undefined}
            aria-describedby={errors.corRaca ? "corRaca-err" : undefined}
          >
        ...
        >
          <select
            ...
            className={inputCls(!!errors.escolaridade)}
            aria-invalid={errors.escolaridade ? true : undefined}
            aria-describedby={
              errors.escolaridade ? "escolaridade-err" : undefined
            }
          >
            <option value="">Selecione</option>
        ...
        <Field label="Identidade de gênero (opcional)" error={undefined}>
          <select {...register("identidadeGenero")} className={inputCls(false)}>
            ...
            {IDENTIDADE_GENERO_OPTIONS.map((o) => (
              <option key={o.value} value={o.value}>
                {o.label}
              </option>
            ))}
        ...
        <Field label="Orientação sexual (opcional)" error={undefined}>
          <select {...register("orientacaoSexual")} className={inputCls(false)}>
            <option value="">Selecione (opcional)</option>
            {ORIENTACAO_SEXUAL_OPTIONS.map((o) => (
              <option key={o.value} value={o.value}>
                {o.label}
              </option>
            ))}

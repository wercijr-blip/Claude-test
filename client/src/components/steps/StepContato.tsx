...
export default function StepContato({
  ...
  const salvar = trpc.paciente.salvarStep3.useMutation({
    onSuccess: () => {
      clearDraft();
      onNext();
    },
  ...
  const buscarCep = async (digits: string) => {
    ...
    if (cached) {
      if (cached.logradouro)
        setValue("logradouro", cached.logradouro, { shouldValidate: true });
      if (cached.bairro)
        setValue("bairro", cached.bairro, { shouldValidate: true });
      if (cached.localidade)
        setValue("cidade", cached.localidade, { shouldValidate: true });
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
      ...
      >
        ...
        {hasIntakeEmail ? (
          <Field label="E-mail">
            ...
            <p className="mt-1 text-xs text-slate-400">
              Preenchido automaticamente do cadastro · não editável
            </p>
          </Field>
        ) : (
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field
            label="CEP"
            error={errors.cep?.message ?? cepErro ?? undefined}
            name="cep"
          >
            <div className="relative">
              <input
                ...
                {...register("cep", {
                  onChange: (e) => {
                    const digits = (e.target.value as string)
                      .replace(/\D/g, "")
                      .slice(0, 8);
          ...
          <Field label="Estado" error={errors.estado?.message} name="estado">
            <select
              ...
              aria-describedby={errors.estado ? "estado-err" : undefined}
            >
        ...
        >
          <input
            ...
            aria-invalid={errors.logradouro ? true : undefined}
            aria-describedby={errors.logradouro ? "logradouro-err" : undefined}
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Bairro" error={errors.bairro?.message} name="bairro">
            <input
              ...
              aria-describedby={errors.bairro ? "bairro-err" : undefined}
            />
          </Field>
          <Field label="Cidade" error={errors.cidade?.message} name="cidade">
            <input
              ...
              aria-describedby={errors.cidade ? "cidade-err" : undefined}
            />
          </Field>
        </div>

...
export default function VerificacaoPage() {
  ...
  const { data, isLoading, error } = trpc.verificacao.consultar.useQuery({
    slug,
  });
    ...
    <div className="min-h-screen bg-warm-bg flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-8 max-w-lg w-full">
        ...
        <p className="text-xs text-slate-500 text-center mb-6">
          Código: <code className="bg-slate-100 px-1 rounded">{slug}</code>
        </p>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-sm text-red-700">
            Erro ao consultar a verificação. Tente novamente em alguns
            instantes.
          </div>
        )}
        ...
        {data && !data.encontrado && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <p className="text-xs text-yellow-700 mt-1 leading-relaxed">
              Este código de verificação não corresponde a nenhum documento
              emitido pelo Facilita PrEP. Se você acredita que é um erro, entre
          ...
          <>
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-xs text-green-700 mt-1 leading-relaxed">
                Este documento foi emitido pelo <strong>Facilita PrEP</strong> e
                assinado digitalmente em padrão{" "}
                <strong>ICP-Brasil PAdES</strong> (Medida Provisória
                2.200-2/2001 · CFM 2.299/2021).
              </p>
            </div>
            ...
            <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-xs font-semibold text-blue-800 mb-2">
                🔍 Verificação completa do PDF
              </p>
              <p className="text-xs text-blue-700 leading-relaxed mb-3">
                Para validar a integridade criptográfica completa do PDF
                (assinatura ICP-Brasil válida e conteúdo não-alterado), faça
                download do arquivo e suba no validador oficial do ITI:
              </p>
              <a
            ...
            <p className="text-[10px] text-slate-400 text-center mt-6 leading-relaxed">
              Por privacidade (LGPD), informações pessoais do paciente não são
              exibidas publicamente. Apenas o paciente e a equipe autorizada têm

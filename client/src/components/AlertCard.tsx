...
> = {
  alto: {
    ...
    badge: "bg-red-50 dark:bg-red-950 text-red-600 dark:text-red-400",
  },
  medio: {
    label: "Médio",
    border: "border-l-amber-400",
    badge: "bg-amber-50 dark:bg-amber-950 text-amber-600 dark:text-amber-400",
  },
  baixo: {
    label: "Baixo",
...
export default function AlertCard({
  ...
  const registrar = trpc.scriba.registrarFeedbackAlerta.useMutation({
    onSuccess: () => {
      toast(
        "Feedback registrado — o CIS aprenderá com esta decisão.",
        "success",
      );
  ...
  return (
    <li
      className={`py-3 border-l-2 pl-3 -ml-4 ${cfg.border}`}
      aria-label={`Alerta de urgência ${cfg.label}: ${alerta.mensagemMedico ?? ""}`}
    >
      {/* Linha principal */}
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2 flex-1 min-w-0">
          <span
            className={`shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded-full capitalize mt-0.5 ${cfg.badge}`}
            aria-hidden="true"
          >
            {cfg.label}
          </span>
          {alerta.mensagemMedico && (
            <p className="text-sm text-stone-700 dark:text-stone-200 leading-snug">
              {alerta.mensagemMedico}
            </p>
          )}
        </div>
          ...
          {jaTemFeedback ? (
            ...
            <button
              ...
              aria-label={
                showFeedback
                  ? "Cancelar feedback"
                  : "Deixar feedback sobre este alerta"
              }
      ...
      {showFeedback && (
        <div
          className="mt-3 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-xl p-3 space-y-3"
          role="form"
          aria-label="Painel de feedback"
        >
          ...
          <div
            ...
            role="group"
            aria-label="Avaliação do alerta"
          >
            {[
              {
                v: "concordo" as const,
              ...
              {
                v: "inaplicavel" as const,
                label: "Não se aplica",
                active:
                  "bg-stone-700 dark:bg-stone-600 text-white border-stone-700",
              },
              {
                v: "discordo" as const,
                label: "Discordo",
              ...
              <button
                ...
                className={[
                  ...
                  tipo === v
                    ? active
                    : "border-stone-200 dark:border-stone-600 text-stone-600 dark:text-stone-300 hover:border-stone-300 bg-white dark:bg-stone-900",
                ].join(" ")}
              >
                {label}
              </button>
